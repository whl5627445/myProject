...
func main() {
	...
	//log.Println(b)
	//log.Println(a)
	a := []byte("{-100.0,-100.0,100.0,100.0,true,-,-,, {Text(true, {0.0, 0.0}, 0.0, {0, 0, 255}, {0, 0, 0}, LinePattern.Solid, FillPattern.None, 0.25, {{-100.0, 100.0}, {-6.0, 72.0}}, \"Heating \"NPN or\" Gate\", 0.0, {-1, -1, -1}, \"\", {}, TextAlignment.Center)}}\n")
...
func parseString(data []byte) string {
	...
	for i := 0; i < lData; i++ {
		if data[i] == '"' && i != 0 {
			...
			//str2Index := bytes.Index(data[i+1:], []byte(","))
