...
def test_getgraphicsdata():
    response = client.get("/simulatemodel/getgraphicsdata", params={"modelname": "Modelica.Blocks.Examples.Filter", "sys_user": "sys"})
    assert response.status_code == 200
    # print(response.json()["data"])
    assert response.json()["data"] == test_getgraphicsdata
    # print ("画图数据： ", response.json()["data"])


def test_getmodelcode():
    response = client.get("/simulatemodel/getmodelcode", params={"modelname": "Modelica.Blocks.Examples.PID_Controller", "sys_user": "sys"})
    assert response.status_code == 200
    assert response.json()["data"] == test_getmodelcode_res
    # print("源码数据： ", response.json()["data"])

def test_getmodelparameters():
    response = client.get("/simulatemodel/getmodelparameters", params={"model_name": "Modelica.Blocks.Examples.PID_Controller", "sys_user": "sys", "components_name": "Modelica.Blocks.Continuous.LimPID", "name": "PI"})
    assert response.status_code == 200
