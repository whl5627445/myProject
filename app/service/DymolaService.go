...
func DymolaFmuExport(fmuPar map[string]interface{}, token, username, fmuName, packageName, modelName, fileName, filePath string) ([]byte, bool) {
	...
	if filePath != "" {
		...
		req.Files = files
		uploadFileRes, err := requests.Post(config.DymolaFmu_CONNECT+"/file/upload", req)
		if err != nil {
			fmt.Println(err)
	...
	exportFmuRes, err := requests.Post(config.DymolaFmu_CONNECT+"/dymola/translateModelFMU", req)
	...
	if res {
		req = url.NewRequest()
		fmuFileUrl := config.DymolaFmu_CONNECT + "/file/download?fileName=" + exportResult["msg"].(string)
