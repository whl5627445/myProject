...
func OmcFmuExportWithLibrary(fmuPar map[string]any, envLibrary map[string]string, userName, fmuName, packageName, modelName, filePath string) (string, bool, string) {
	...
	if filePath != "" {
		...
		req.Files = files
		uploadFileRes, err := requests.Post(config.OmcFlaskConnect+"/file/upload", req)
		if err != nil {
			log.Println(err)
			return "", false, errTips
		}
	...
	req.Headers = Headers
	req.Timeout = time.Minute * 10
	exportFmuRes, err := requests.Post(config.OmcFlaskConnect+"/omc/buildModelFMU", req)
	if err != nil {
		errTips = "下载失败，请查看日志"
		messageMap["message"] = "Description Failed to send the http request"
		messageMap["status"] = true
	...
	req.Headers = Headers
	req.Timeout = time.Minute * 10
	fmuFileUrl := config.OmcFlaskConnect + "/file/download?fileName=" + exportResult["msg"].(string)
	fmuFileRes, err := requests.Get(fmuFileUrl, req)
	if err != nil {
		return "", false, errTips
	}
