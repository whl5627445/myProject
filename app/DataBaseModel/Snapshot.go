...
type YssimSnapshots struct {
	ID                string         `gorm:"index;primaryKey;type:varchar(128);comment:视图快照的唯一标识"`
	SpaceId           string         `gorm:"index;column:space_id;type:varchar(128);comment:空间id"`
	PackageId         string         `gorm:"index;column:package_id;type:varchar(128);comment:包id"`
