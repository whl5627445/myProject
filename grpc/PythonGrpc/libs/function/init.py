from config.db_config import Session, YssimSimulateRecords, YssimModels, AppDataSources, ParameterCalibrationRecord
import router_pb2
...
def initcalibrationTask():
    ...
    with Session() as session:
        task_record_list = []
        record_list = session.query(ParameterCalibrationRecord).filter(
        ...
        log.info("(标定)未完成的任务记录：" + str([k.id for k in record_list]))
        for i in record_list:
            yssim_model = session.query(YssimModels).filter(
                YssimModels.userspace_id.in_([i.userspace_id, '0']),
                YssimModels.id == i.package_id,
    ...
    # 仿真任务列表
    for i in task_record_list:
        if i.compile_status == "6" or i.compile_status == "1" or i.compile_status == "2":
            a = router_pb2.SubmitTaskRequest(
                uuid=i.id,
                userSpaceId=i.user_space_id,
                userName=i.username,
